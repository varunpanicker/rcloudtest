...
RUN apt-get install -y --no-install-recommends gettext redis-server rsync curl lisbxml2-dev python-dev vim && rm -rf /var/lib/apt/lists/*
...
RUN ["chmod", "x", "/data/rcloud/scripts/run-rcloud.sh"]
